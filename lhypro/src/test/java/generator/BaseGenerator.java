...
public abstract class BaseGenerator {
	...
	public void generate() {
		...
		if (containsGenType(GenType.HTML) && StringUtils.isBlank(this.genConfig.saveDirForHtml)) {
			throw new IllegalArgumentException("生成HTML时需要设置SaveDirForVue参数");
		}
		try {
	...
	protected String mysqlProcessType(String type) {
		...
		System.err.println(new StringBuilder().append("unown type [").append(type).append("]").toString());
		return null;
	}
	...
	protected String oracleProcessType(String type) {
		...
		System.err.println(new StringBuilder().append("unkown type [").append(type).append("]").toString());
		return null;
	}
	...
	protected boolean isDecimal(List<String> types) {
		for (Iterator localIterator = types.iterator(); localIterator.hasNext(); ) {
			String type = (String) localIterator.next();
			if (type.toLowerCase().contains("decimal") || type.toLowerCase().contains("numeric"))
				return true;
		}
		return false;
	}
